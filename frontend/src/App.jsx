...
import { BookOpen, Server, Workflow, Briefcase, RefreshCw, Plus, PlayCircle } from "lucide-react";
...
function QuickAction({ icon: Icon, label, onClick }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-2 bg-slate-800 border border-slate-700 hover:border-cyan-500/50 hover:bg-slate-750 rounded-xl px-4 py-3 text-slate-200 text-sm font-medium transition"
    >
...
function Dashboard({ token, refreshKey, onNavigate }) {
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        <QuickAction icon={Plus} label="New Workbook" onClick={() => onNavigate("workbooks")} />
        <QuickAction icon={Plus} label="New Inventory" onClick={() => onNavigate("inventory")} />
        <QuickAction icon={Plus} label="New Flow" onClick={() => onNavigate("flows")} />
        <QuickAction icon={PlayCircle} label="Run Job" onClick={() => onNavigate("jobs")} />
      </div>
